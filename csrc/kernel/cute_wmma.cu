...
__global__ void my_cute_gemm(const half_t* A, const half_t* B, float* C) {
    using namespace cute;
...
int main() {
    ...
    std::vector<half_t> hA(ElementA);
    std::vector<half_t> hB(ElementB);
    std::vector<float> hC(ElementC);
    std::vector<float> ref(ElementC);

    for (int m = 0; m < kM; ++m) {
        for (int k = 0; k < kK; ++k) {
            hA[m * kK + k] = half_t(static_cast<float>((m + 1) * 0.25f + k));
        }
    }

    for (int k = 0; k < kK; ++k) {
        for (int n = 0; n < kN; ++n) {
            hB[k * kN + n] = half_t(static_cast<float>((n + 1) * 0.125f - k * 0.5f));
        }
    }
    ...
    for (int m = 0; m < kM; ++m) {
        for (int n = 0; n < kN; ++n) {
            float acc = 0.0f;
            for (int k = 0; k < kK; ++k) {
                acc += float(hA[m * kK + k]) * float(hB[k * kN + n]);
            }
            ref[m * kN + n] = acc;
        }
    }
    ...
    half_t* dB;
    float* dC;
    CHECK_CUDA(cudaMalloc(&dA, ElementA * sizeof(half_t)));
    CHECK_CUDA(cudaMalloc(&dB, ElementB * sizeof(half_t)));
    CHECK_CUDA(cudaMalloc(&dC, ElementC * sizeof(float)));

    CHECK_CUDA(cudaMemcpy(dA, hA.data(), ElementA * sizeof(half_t),
                        cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(dB, hB.data(), ElementB * sizeof(half_t),
                        cudaMemcpyHostToDevice));
    ...
    auto tiled_mma =
      make_tiled_mma(SM80_16x8x8_F32F16F16F32_TN{},
                     Layout<Shape<_1, _4, _1>>{});
